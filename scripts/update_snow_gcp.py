#!/usr/bin/env python3
"""Update ServiceNow RITM for GCP Infrastructure requests."""
...
def build_payload(mode: str) -> dict:
    ...
    project = get_env("PROJECT_NAME", required=False) or "unknown"
    gcp_folder = get_env("GCP_PROJECT_FOLDER", required=False) or f"{project}"
    ...
    if mode == "pr_created":
        return {
            ...
            "work_notes": (
                f"Pull Request created for GCP infrastructure provisioning.\n"
                f"GCP Project : {gcp_folder}\n"
                f"PR URL      : {pr_url}\n"
                f"Created at  : {stamp}\n"
                f"Awaiting infra team review and merge."
            ),
            "comments": (
                f"Your GCP infrastructure request is progressing!\n\n"
                f"A Pull Request has been raised in the gcp-infrastructure repository:\n"
                f"  PR: {pr_url}\n\n"
                f"GCP Project: {gcp_folder}\n\n"
                f"What happens next:\n"
                f"  - The Infrastructure team will review and approve the PR\n"
                f"  - Once merged, Terraform will automatically provision your GCP resources\n"
    ...
    if mode == "deploy_success":
        ...
        return {
            ...
            "close_notes": f"GCP infrastructure deployed successfully: {gcp_folder}",
            "work_notes": (
            ...
            "comments": (
                f"Great news! Your GCP infrastructure has been deployed successfully.\n\n"
                f"GCP Project : {gcp_folder}\n"
                f"Deployed at : {stamp}\n"
                f"{resources_section}"
                f"Your resources are now live and ready to use.\n\n"
                f"You can view your resources in the Google Cloud Console.\n"
    ...
    if mode == "deploy_failed":
        return {
            "state": "2",  # Work in Progress
            "work_notes": (
                f"Terraform apply FAILED for GCP project: {gcp_folder}\n"
                f"Failed at: {stamp}\n"
                f"Action required: Infrastructure team investigation needed."
            ),
            "comments": (
                f"Unfortunately, the GCP infrastructure deployment has failed for project: {gcp_folder}\n\n"
    ...
    if mode == "failed":
        return {
            "state": "2",  # Work in Progress
            "work_notes": (
                f"GCP infrastructure provisioning FAILED.\n"
                f"Target project: {gcp_folder}\n"
                f"Failed at: {stamp}\n"
                f"Action required: Platform team investigation needed."
            ),
            "comments": (
                f"Unfortunately, GCP infrastructure provisioning has failed.\n"
                f"Please check with Platform Team: platformsupport@nexturn.com\n\n"
