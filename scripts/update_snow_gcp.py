...
def build_payload(mode: str) -> dict:
    ...
    folder = get_env("PROJECT_FOLDER", required=False) or project
    ...
    if mode == "deploy_success":
        resources_section = f"\nProvisioned Resources:\n{resource_details}\n" if resource_details else ""
        return {
            "state": "3",
            "close_notes": f"GCP infrastructure deployed successfully: {folder}",
            "work_notes": (
    ...
    if mode == "deploy_failed":
        return {
            "state": "2",
            "work_notes": (
                f"Terraform apply FAILED for GCP project folder: {folder}\n"
                f"Failed at: {stamp}\n"
                f"Action required: Infrastructure team investigation needed."
            ),
            "comments": (
                f"Unfortunately, the GCP infrastructure deployment has failed for: {folder}\n\n"
    ...
    if mode == "failed":
        return {
            ...
            "comments": (
                "Unfortunately, GCP infrastructure provisioning has failed. "
                "Please contact platformsupport@nexturn.com"
            ),
        }
